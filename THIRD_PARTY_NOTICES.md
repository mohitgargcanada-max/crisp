...
| **Andrej Karpathy Skills** | [multica-ai/andrej-karpathy-skills](https://github.com/multica-ai/andrej-karpathy-skills) (fork of forrestchang's original) | **MIT** (declared in upstream `.claude-plugin/plugin.json`; no separate LICENSE file upstream) | **Vendored as-is** — `claude/skills/karpathy-guidelines/SKILL.md` is the actual upstream skill, unmodified. It's a single self-contained skill with no cross-references to sibling skills, so — unlike Superpowers below — it copies in safely as a flat file. MIT notice reproduced at `claude/skills/karpathy-guidelines/LICENSE`. Replaces the previously-vendored `lean-code-agent` (removed 2026-08-23, see CREDITS.md). |
...
- **Graphify and Andrej Karpathy Skills** are verbatim vendored copies of another project's licensed files — each keeps its own MIT license+copyright inside its own skill folder (`graphify/LICENSE`, `karpathy-guidelines/LICENSE`).
- **Superpowers, Code Review, and Claude-Mem are real Claude Code plugins** — namespaced skills and/or their own hooks that would break if copied in as flat files. CRISP documents and install-checks them (`install.ps1`/`install.sh`), it doesn't vendor them. Karpathy's skill is the exception: despite also being packaged as a plugin upstream, its actual skill content is a single file with no internal cross-references, so vendoring it directly is safe (unlike Superpowers' 14 interlinked skills).
